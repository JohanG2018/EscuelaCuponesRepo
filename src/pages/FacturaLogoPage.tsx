import { Button } from "primereact/button";
import { Card } from "primereact/card";
import { FileUpload } from "primereact/fileupload";
import { MultiSelect } from "primereact/multiselect";
import { InputText } from 'primereact/inputtext';

export default function FacturaLogoPage() {
  return (
    <div className="mx-auto  space-y-4">
        <h1 className="text-2xl text-center p-2 font-semibold">Asignar Logo de Factura</h1>
      {/* Subida de nuevo logo */}
      <div className="flex flex-col">
        <label className="font-semibold p-2" htmlFor="">Nombre del logo</label>
        <InputText placeholder="Ingrese el nombre del logo de la factura"></InputText>
      </div>
      <div>
        <label className="font-semibold block mb-2">Cargar nuevo logo (.bmp, máx 600px ancho)</label>
        <FileUpload
          name="logo"
          accept=".bmp"
          maxFileSize={5 * 1024 * 1024}
          customUpload
          chooseLabel="Seleccionar archivo"
          uploadLabel="Cargar archivo"
          cancelLabel="Cancelar"
          emptyTemplate={<p className="m-0">Arrastre el archivo aquí o haga clic</p>}
        />
      </div>

      {/* Selección de locales */}
      <div>
        <label className="font-semibold block mb-2">Locales a asignar</label>
        <MultiSelect
          options={[]} // ← llenar luego
          optionLabel="local"
          placeholder="Seleccione uno o varios locales"
          className="w-full"
        />
      </div>

      {/* Botón de guardar */}
      <div className="flex justify-end">
        <Button
          label="Guardar"
          icon="pi pi-save"
          className="bg-green-600 text-white px-5 py-2"
        />
      </div>
      {/* Vista previa del logo actual */}
      <div>
        <h3 className="font-semibold mb-2">Logo asignado actualmente:</h3>


      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[1, 2, 3].map((i) => (
          <Card
            key={i}
            className="p-4 shadow-md rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900"
          >
            <div className="flex flex-col items-center gap-4">
              {/* Imagen del logo */}
              <div className="border rounded-md bg-white shadow p-2">
                <img
                  src="/img/placeholder_logo.bmp"
                  alt={`Logo ${i}`}
                  className="h-24 w-auto object-contain"
                />
              </div>

              {/* Locales */}
              <div className="text-center">
                <h3 className="text-base font-semibold mb-1">Locales</h3>
                <ul className="list-disc list-inside text-sm text-gray-700 dark:text-zinc-200">
                  <li>Local {i}-1</li>
                  <li>Local {i}-2</li>
                  <li>Local {i}-3</li>
                </ul>
              </div>
            </div>
          </Card>
        ))}
      </div>

    </div>
  );
}
