...
const CuponesPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (successMsg && !yaMostrado.current) {
      toast.current?.show({
        severity: "success",
        summary: "Éxito",
        detail: successMsg,
  ...
  const onToggleEstado = async (cupon: Cupon) => {
    ...
    const snapshot = cupons.map(c => ({ ...c }));

    setCupons(prev =>
      prev.map(c => c.id === cupon.id ? { ...c, estado: next } : c)
    );
    ...
    try {
      ...
      toast.current?.show({
        ...
        detail: `El cupón ${cupon.id} ahora está ${next ? "Activo" : "Inactivo"}.`,
        life: 2500,
      });
    } catch (err: any) {
